...
function preprocess(
  ...
  const { expandTabs, nul, tabSize = 2 } = options ?? {}
  return preprocessor
  ...
  function preprocessor(
      ...
      while (index < value.length) {
        ...
        switch (true) {
          case code === codes.cr:
            ...
            column = 1
            break
          case code === codes.ht && expandTabs !== false:
            /**
             * Next column.
             *
             * @const {number} n
             */
            const n: number = (Math.ceil(column / tabSize) * tabSize) - column
          ...
          case code === codes.lf:
            ...
            column = 1
            break
          case code === codes.nul && nul === false:
            chunks.push(codes.replacement)
            column++
            break
          default:
            chunks.push(code)
    ...
    return chunks.push(codes.eof), chunks
  }
}
